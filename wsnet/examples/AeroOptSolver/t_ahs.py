...
class TAHS:
    """
    Two-Stage Adaptive Hybird Surrogate (T-AHS) Model.
    ...
    def __init__(self, threshold: float = 0.5, prs_degree: int = 2, rbf_centers: int = 20,
                 svr_c: float = 1.0, svr_epsilon: float = 0.1,
                 krg_poly: str = 'constant', krg_kernel: str = 'gaussian'):
        """
        ...
        - prs_degree (int): Degree of the PRS component.
        - rbf_centers (int): Number of centers for the RBF component.
        - svr_c (float): Regularization parameter for the SVR component.
        - svr_epsilon (float): Epsilon-tube width for the SVR component.
        - krg_poly (str): Regression function for the KRG component.
        - krg_kernel (str): Correlation function for the KRG component.
        """
        self.threshold = threshold

        # Initialize the pool of component models
        self.models_pool = [
            PRS(degree=prs_degree),
            RBF(num_centers=rbf_centers),
            KRG(poly=krg_poly, kernel=krg_kernel),
            SVR(C=svr_c, epsilon=svr_epsilon)]
        self.model_names = ['PRS', 'RBF', 'KRG', 'SVR']

        # Dedicated model for variance estimation
        self.uncertainty_model_ = KRG(poly=krg_poly, kernel=krg_kernel)
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray) -> None:
        """
        Trains the Two-Stage Adaptive Hybird Surrogate (T-AHS) Model.
