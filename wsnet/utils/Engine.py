...
class AutoregressiveTrainer(BaseTrainer):
    ...
    2. Noise injection: adds Gaussian noise to inputs during rollout to improve stability.
    """
    ...
    def _parse_batch(self, batch: Union[List[Tensor], Tuple[Tensor, ...], Tensor], rollout_steps: int
                    ) -> Tuple[Tensor, Optional[Tensor]]:
        """
        Intelligently parses the batch to extract sequence and optional coordinates.

        Args:
        - batch: Raw batch from DataLoader. Could be [seq], [seq, coords], or just seq Tensor.
        - rollout_steps (int): The number of prediction steps required.

        Returns:
        - seq_window (Tensor): Shape (batch_size, rollout_steps + 1, num_nodes, num_channels)
        - coords (Optional[Tensor]): Shape (batch_size, num_nodes, spatial_dim) or None
        """
        seq: Tensor
        ...
        if isinstance(batch, (list, tuple)):
            if len(batch) == 1:
                ...
                seq, coords = batch[0], batch[1]
            else:
                raise ValueError(f'Unexpected batch length: {len(batch)}. Expected 1 (seq) or 2 (seq, coords)')
        ...
        # 2. Validate shapes
        if seq.ndim != 4:
            raise ValueError(f'Sequence tensor must have 4 dimensions, got {seq.shape}.')
        if coords is not None:
            if coords.ndim != 3:
                raise ValueError(f'Coords tensor must have 3 dimensions, got {coords.shape}.')

        # 3. Random slicing logic
        total_len = seq.shape[1]
        required_len = rollout_steps + 1

        if total_len < required_len:
            raise ValueError(f'Sequence length ({total_len}) is too short for {rollout_steps} rollout steps.')

        if self.model.training:
            max_start_idx = total_len - required_len
            if max_start_idx > 0 :
        ...
        seq_window = seq[:, start_idx : start_idx + required_len]

        return seq_window, coords
    ...
    def compute_loss(self, batch: Any) -> Tensor:
        ...
        - batch (Any): Data batch containing sequence and optionally coords.
                       Sequence shape: (batch_size, seq_len, num_nodes, num_channels)
                       Coordinates shape: (batch_size, num_nodes, spatial_dim)

        Returns:
        - Tensor: Scalar loss averaged over rollout steps. Shape (1,)
        """
        # 1. Determine rollout horizon
        steps = self._get_current_steps()

        # 2. Parse batch and get a random time window
        seq_window, coords = self._parse_batch(batch, steps)
        current_state = seq_window[:, 0]
