...
class CFDataset(Dataset):
    ...
    def build_datasets(split_ratio: Tuple[float, float, float] = (0.8, 0.1, 0.1),
                       ...
                       win_len: int = 16, win_stride: int = 1
        ...
        def create_windows(dataset: 'CFDataset', split_name: str) -> None:
            """Vectorized sliding window for data augmentation across all cases."""
            seq_win: List[Tensor] = []
            ...
            for seq, coords in pbar:
                ...
                if seq.shape[0] < win_len:
                    continue

                # Unfold seq: (win_size, win_len, num_nodes, num_channels)
                seq_unfold = seq.unfold(dimension=0, size=win_len, step=win_stride).permute(0, 3, 1, 2)

                for i in range(seq_unfold.shape[0]):
                    seq_win.append(seq_unfold[i])
                    coords_win.append(coords)

            # Shuffle across all cases
            combined = list(zip(seq_win, coords_win))
            rng.shuffle(combined)

            # Update the Dataset object internal buffers
            dataset.sequences, dataset.coords = zip(*combined) if combined else ([], [])
